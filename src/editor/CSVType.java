...
public class CSVType implements DocumentType {
	 ...
	 * document. For example, <code>"Image/Resize"</code> could be one such menu
	 * element for an image processing program. The descriptor must not include
	 * type-independent menu items, such as <code>"File/Exit"</code> Normally
	 * the actual {@link ca.queensu.cs.dal.edfmwk.menu.MenuDescriptor} would
	 * have been static, but specific actions such as Cut and Paste must be
	 * fetched from the {@link javax.swing.JTextArea} embedded in the frame
	 * displaying the document.
	 * 
	 * @param doc
	 *            Document whose state or GUI representation might influence the
	...
	public MenuDescriptor getMenu(Document doc) {
		...
		for (int i = 0; i < actionPairs.length; i++) {
			...
			try {
				...
				System.out.println("Path error " + menuName + "=" + actionName + ":" + e);
			}
		} // for
			// JTextArea-specific actions
	 ...
	 * System.out.print(name); if (km!=null) { KeyStroke strokes[] =
	 * km.getKeyStrokesForAction(ac); if (strokes == null) { System.out.print(
	 * " no keystrokes"); } else if (strokes.length==0) { System.out.print(
	 * " zero-length keystrokes"); } else { for(int j=0; j<strokes.length; j++)
	 * { System.out.print(" "+strokes[j]); } } } System.out.println(); }
	 */

	private static String[][] actionPairs = { { "Edit/Copy", DefaultEditorKit.copyAction },
			{ "Edit/Cut", DefaultEditorKit.cutAction },
			// { "", DefaultEditorKit.cutAction}, // test error check
			{ "Edit/Paste", DefaultEditorKit.pasteAction } };

	/**
	 * Map from action names to text-component-specific actions. It should only
	 * be considered valid within a single call to {@link #getMenu}
	 */
	...
	/**
	 * Set the {@link #actions} member to contain a list of the actions allowed
	 * on the current text component. The actions (might?) embed references to
	 * the specific text component, which is why we have to do it over again for
	 * each document.
	 * 
	 * @param txt
	 *            The text component from which to retrieve actions.
	 */
	 ...
	 * document. For example, <code>"Image/Resize"</code> could be one such menu
	 * element for an image processing program. The descriptor must not include
	 * type-independent menu items, such as <code>"File/Exit"</code>
	 */
	...
	/**
	 * Get the filename extensions appropriate for this kind of document. For
	 * example, <code>{ ".html", ".htm" }</code> for HyperText Markup Language
	 * documents.
	 */
	 ...
	 */
	private static String[] extensions = { "csv" }; // Updated to allow only csv
													// instead of html and txt
